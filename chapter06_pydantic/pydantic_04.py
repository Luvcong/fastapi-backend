...
EmailStr : 이메일주소 Validate 검증
- email-validator 라이브러리 사용 필요 (pip install 'pydantic[email]')
"""
...
class UserEmail(BaseModel) :
    # 1) 기본 이메일 문자열 검증
    email : EmailStr    # 이메일 문자열 검증

    # 2) 기본 이메일 검증 + 최대 길이 검증
    # email : EmailStr = Field(..., max_length=40)

    # 3) 기본 이메일 검증 + 최대 길이 + 정규식 검증
...
try :
    ...
    # user_email = UserEmail(email = 'userexamples.com')    # 이메일 형식에 맞지 않으므로 에러 발생
...
try:
    user_resource = UserResource(
        ...
        # any_url = 'ftp//example.com', # 콜론(:)이 없으면 에러 발생
