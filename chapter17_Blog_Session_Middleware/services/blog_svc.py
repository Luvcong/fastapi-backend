...
async def get_blog_by_id(conn : Connection, id: int) :
    try :
        ...
        query = f'''select a.id, title, author_id, b.name as author, b.email as email, content, image_loc, modified_dt
                      from blog a
                      join user b on author_id = b.id
                     where a.id = :id'''   # bind variable 사용
        ...
        blog = BlogData(id = row.id, title = row.title, author = row.author, author_id = row.author_id, email = row.email, 
                        content = row.content, image_loc = row.image_loc, modified_dt = row.modified_dt)
...
async def create_blog(conn : Connection, title : str, author_id : int, content : str, image_loc = None) :
    try :
        query = f'insert into blog(title, author_id, content, image_loc, modified_dt) values ("{title}", {author_id} , "{content}", {util.none_to_null(image_loc, is_squote = True)} , now())'
...
# 블로그 게시글 수정
async def update_blog(conn : Connection, id : int, title : str, content : str, image_loc : UploadFile) :
    try :
        query = f'''
            update blog
               set title = :title
