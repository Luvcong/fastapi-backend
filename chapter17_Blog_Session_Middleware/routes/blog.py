...
# 블로그 데이터 전체 조회
@router.get('/')
async  def get_all_blogs(request : Request, conn: Connection = Depends(context_get_conn), session_user = Depends(auth_svc.get_session_user_opt)) :
...
# 블로그 데이터 단건 조회
@router.get('/show/{id}')
async def get_blog_by_id(request: Request, id: int, conn : Connection = Depends(context_get_conn), session_user = Depends(auth_svc.get_session_user_opt)) :
    ...
    return templates.TemplateResponse(
        request = request,
        name = 'show_blog.html',
        context = {'blog' : blog, 'session_user' : session_user, 'is_valid_auth' : is_valid_auth})

# 블로그 게시글 신규 생성 템플릿 양식 조회
@router.get('/new')
async def create_blog_ui(request: Request, session_user = Depends(auth_svc.get_session_user_prt)) :
    return templates.TemplateResponse(
        request = request,
        name = 'new_blog.html',
        context = {'session_user' : session_user}    # 전달 값이 없는 경우 빈 dict 객체 전달
    )
...
async def update_blog_ui(request : Request, id : int, conn = Depends(context_get_conn), session_user = Depends(auth_svc.get_session_user_prt)) :
...
async def update_blog(request : Request
    ...
    if not is_valid_auth :
        raise HTTPException(status_code = status.HTTP_401, detail = '해당 서비스는 권한이 없습니다.')
...
async def delete_blog(request: Request, id: int, conn = Depends(context_get_conn), session_user = Depends(auth_svc.get_session_user_prt)) :
    blog = await blog_svc.get_blog_by_id(conn = conn, id = id)
    is_valid_auth = auth_svc.check_valid_auth(session_user, blog_author_id = blog.author_id, blog_email = blog.email)

    if not is_valid_auth :
        raise HTTPException(stauts_code = status.HTTP_401_UNAUTHORIZED, detail = '해당 서비스는 권한이 없습니다.')
...
async def get_blog_by_id_json(request : Request, id : int, conn = Depends(context_get_conn)) :
    blog = await blog_svc.get_blog_by_id(conn = conn, id = id)
    blog.content = util.newline_to_br(blog.content)
